...
interface FixtureRecord extends FixtureCase {
  php: unknown;
}
...
function decodeJsonFromPhpOutput(text: string): unknown {
  for (let i = 0; i < text.length; i += 1) {
    ...
    try {
      ...
      // keep searching until we find a parsable JSON payload
    }
  }
...
function runPhp(caseDef: FixtureCase): unknown {
...
function main(): void {
  const fixtures: FixtureRecord[] = CASES.map((caseDef) => ({
    ...caseDef,
    php: runPhp(caseDef)
  }));

  mkdirSync(pathDirname(OUTPUT_PATH), { recursive: true });
  writeFileSync(OUTPUT_PATH, `${JSON.stringify({ fixtures }, null, 2)}\n`, "utf8");

  process.stdout.write(`Generated ${fixtures.length} fixtures\n`);
}
