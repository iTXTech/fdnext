...
| `1G/2G/4G/8G` | density | 1Gb/2Gb/4Gb/8Gb |
...
当前只落地外部可确认的 `H5CG48AGBD-X018` 结构：
...
- `8` 输出 x8。
- `A` 输出 `A-die`。
- `GB` / `X018` 结合外部样本输出 `DDR5-5600`。
- `X018` 输出为 `package_code`；顶层 `package` 只输出已由外部资料确认的 `BGA`。
- TechInsights 明确该 package 内是 single 16Gb die，因此输出 `dram_die_stack = Single die`。
