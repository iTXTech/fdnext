...
- `packages/resources/resources/micron-dram-fbga-codes.json` 只保存一次性提取的 5 位 DRAM FBGA code，并排除 `crawl-mdb` 已覆盖的 Micron NAND 段 `NC/NW/NY/NX/NQ/NV`；`pnpm fdbgen:crawl-mdb-from-fbga` 读取该 code list，通过 Micron 官方 FBGA decoder API 写入统一 `packages/resources/resources/mdb.json`。
