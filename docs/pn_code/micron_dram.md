...
- `pnpm fdbgen:crawl-mdb-from-fbga` 默认按 `C9/D8/D9/Z8/Z9` + 字母网格生成 Micron DRAM FBGA 候选，通过 Micron 官方 FBGA decoder API 写入统一 `packages/resources/resources/mdb.json`。`references/micron-fbga-codes.json` 仅作为非默认网格的补充参考 code 输入，不打包进 resources 包，并排除 `crawl-mdb` 已覆盖的 Micron NAND 段 `NC/NW/NY/NX/NQ/NV`。
