...
本报告记录从 Phison 官方 support list JSON（`/Users/peratx/dev/firstchip-cfg-editor/ps.json`）中抽出的候选规则，以及这些候选没有加入 `packages/decodepack/src/rules/packs/phison-token.json` 的原因。
