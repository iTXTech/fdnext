...
- Micron 官方 M510 / M500IT / 5400 / 6500 ION ordering 图：确认 M510/M500IT 的 `AZ/BD` 系列 token、M500IT 的 `032` 容量与 `IT` 工业等级；确认 5400 的 `GA/GB/GC` 系列、`BC = 512Gb TLC x8 2.5V (3D)`；确认 6500 ION 的 `K = PCIe Gen4`、`CC/BN` form factor、`30T7 = 30,720GB`、`GR = 6500 ION`、sector size 和 OCP firmware token。
...
| NAND type `S/M/T` | SLC / MLC / TLC；`T` 主要来自 2100AI/AT 资料 |
| production status `ES/MS` | Engineering samples / Mechanical samples |
...
M500IT chart 中 `AH/AK` component token 只给出 density、x8、3.3V 和 20nm，没有稳定声明 SLC/MLC；公开 `cell_level` 由 NAND type token `S/M` 决定，component 文本不重复 cell level。
