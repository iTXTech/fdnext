...
- 规则文件：`packages/dsl/src/rules/packs/samsung-dram-token.json`
...
- 当前覆盖：
  ...
  - DDR5：`K4R`，当前落地 `AH` density token。
  - LPDDR：`K4X/K3P/K3Q/K4F/K4U/K3L`，覆盖 LPDDR、LPDDR2、LPDDR3、LPDDR4、LPDDR4X、LPDDR5 的已确认 token。
...
- 标准 DDR/GDDR 颗粒在 datasheet 或官方页面确认单颗 die / 单 rank 语义时输出 `Single die, 1 CS`。
