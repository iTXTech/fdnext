...
- LPDDR4X `3/4/5/6/B/C/D` 分别输出 8/16/32/64/24/48/96Gb；package type `A/C` 当前均输出 200-ball FBGA，`6:CDM` 进一步输出 `FBGA-200, 10x15x0.78`；config `A/B/C` 当前均输出 x32；`BAM` 输出 `2 dies, 1 CS`，`CAM` 与 `CBM` 输出 `4 dies, 2 CS`，`CCM` 输出 `6 dies, 2 CS`，`CDM` 输出 `8 dies, 2 CS`。`D:CCM` 组合输出 `CXMT G4`，其余既有组合继续按已确认映射输出。
