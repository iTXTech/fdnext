...
- CXMT：按 Rockchip/CSEKER 外部料号表补入 `CXDC/CXDD` LPDDR5/LPDDR5X family、density、layout/package 局部 token 与搜索 PN；按 EDN 实物拆解与开发板手册补入 `CXDQ2BFAM-CG` 的 4Gb x16 / DDR4-2400 组合 token；由 CSEKER、Rockchip QVL、ChromeOS non-SPD table、FCC BOM 与野火官方原理图交叉确认 8 条 LPDDR4X exact PN；本轮再从公开颗粒表、TechInsights 实物页和器件页补入 8 条 standalone exact 搜索 PN，并按 `6:CDM` 结构组合补齐 10x15x0.78 封装，不据完整料号改写 decoder。容量或封装相互冲突的 `CXDB7CCDM-MA-M` / `CXDQ2BFAM-CE-B` 继续排除。
