...
| 面向 | 当前证据 |
| --- | --- |
| DRAM 搜索资源 | `dram-pn.json` 共 3117 条，重复 PN 为 0；vendor 缺失为 0。`H57G42MP4AX004N` / `H5WG6HMN6QX038R` 只有单一公开 body，作为 exact 搜索种子保留，但不据此建立 decoder，因此 type / numeric density 明确缺失 2 条 |
| Managed NAND 搜索资源 | `managed-nand-pn.json` 共 886 条，重复 PN 为 0；vendor 缺失为 0。`HN8T039JHQX099N` 只有单一公开 body，作为 exact 搜索种子保留，不据此建立 decoder，因此 type / numeric density 明确缺失 1 条 |
...
- SK hynix：HBM2E/HBM3、32Gb DDR4、modern LPDDR4X、eMCP 独立 storage/DRAM token；按 byte3 条件化补入 legacy SLC Read ID geometry，并用 H27 2Gbit datasheet 锁定四种电压/位宽回归；本轮从公开产品表补入 153 条 standalone DRAM 与 12 条 UFS exact 搜索 PN，新增结构化 H58 LPDDR5 与 H56G32 8Gb GDDR6 规则，扩展 LPDDR5X `GD/GE` density 与 315/441/561-ball serial package，并确认 H9HC `BP/T`、H54 `AYR:B/J`、H28S package 语义。产品表中的模组与 SSD 页面明确排除。
