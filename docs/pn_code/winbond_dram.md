...
本页记录 Winbond standalone DRAM 颗粒的 PN 结构。本轮覆盖官方 2026 PSG 中可直接确认的 DDR3、DDR4、LPDDR4/LPDDR4X；SDR、DDR、DDR2、LPDDR3 等已在 PSG 中出现，但未在本轮落 DSL。
...
- 规则文件：`packages/dsl/src/rules/packs/winbond-dram-token.json`
...
- `packages/resources/resources/dram-pn.json` 收录本轮 DSL 能解析的官方 Winbond PN 样例，用于搜索补全；解码仍由 token 规则完成。
