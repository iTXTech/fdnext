...
export interface FdServerOptions {
  host?: string;
  port?: number;
  engine?: FdnextEngine;
...
function requestLang(request: Request, fallback: LegacyLang): LegacyLang {
  return parseLang(request.url.searchParams.get("lang"), fallback);
}
...
function parseExtraUrls(env: NodeJS.ProcessEnv, warn?: (message: string) => void): Record<string, string> {
...
function createConfig(options: FdServerOptions): FdServerConfig {
...
function hasQueryParam(request: Request, key: string): boolean {
  return request.url.searchParams.has(key);
}

function queryParam(request: Request, key: string): string {
  return request.url.searchParams.get(key) ?? "";
}

function positiveLimit(request: Request): number | undefined {
  const raw = request.url.searchParams.get("limit")?.trim();
  if (!raw) {
    return undefined;
  }
...
function json(h: ResponseToolkit, body: unknown, code = 200) {
  return h.response(body as Parameters<ResponseToolkit["response"]>[0]).code(code)
    .type("application/json")
    .header("Access-Control-Allow-Origin", "*")
    .header("Access-Control-Allow-Headers", "*");
}

async function handleLegacyRequest(engine: FdnextEngine, config: FdServerConfig, request: Request, h: ResponseToolkit) {
  const path = cleanPath(request.url.pathname);
  const lang = requestLang(request, config.defaultLang);

  if (path === "/") {
    return json(h, { result: true, time: Math.floor(Date.now() / 1000), server: FD_SERVER_NAME });
  }
  if (path === "/info") {
    return json(h, legacyInfo(engine));
  }
  if (path === "/decode") {
    if (!hasQueryParam(request, "pn")) {
      return json(h, { result: false, message: "Missing part number" });
    }
    const pn = queryParam(request, "pn");
    const result = engine.decodePart({ query: pn, lang, controllerGroup: config.controllerGroup });
    return json(h, { result: true, data: toLegacyFlashInfo(result, pn, lang, config.extraUrls) });
  }
  if (path === "/decodeId") {
    if (!hasQueryParam(request, "id")) {
      return json(h, { result: false, message: "Missing flash id" });
    }
    ...
    return json(h, { result: true, data: toLegacyFlashIdInfo(result, id, lang, config.extraUrls) });
  }
  if (path === "/searchPn") {
    if (!hasQueryParam(request, "pn")) {
      return json(h, { result: false, message: "Missing part number" });
    ...
    const pn = queryParam(request, "pn");
    const limit = positiveLimit(request);
    const result: PartSearchResult = engine.searchParts({ query: pn, lang, ...(limit ? { limit } : {}) });
    return json(h, { result: true, data: result.items.map((item) => legacySearchPart(item, lang)) });
  }
  if (path === "/searchId") {
    if (!hasQueryParam(request, "id")) {
      return json(h, { result: false, message: "Missing flash id" });
    ...
    return json(h, { result: true, data });
  }
  if (path === "/summary") {
    if (!hasQueryParam(request, "pn")) {
      return json(h, { result: false, message: "Missing part number" });
    ...
    const pn = queryParam(request, "pn");
    const result = engine.decodePart({ query: pn, lang, controllerGroup: config.controllerGroup });
    return json(h, { result: true, data: partSummary(toLegacyFlashInfo(result, pn, lang, config.extraUrls)) });
  }
  if (path === "/summaryId") {
    if (!hasQueryParam(request, "id")) {
      return json(h, { result: false, message: "Missing flash id" });
    ...
    const id = queryParam(request, "id");
    const result = engine.decodeIdentifier({ query: id, lang, idScheme: "nand.flash_id", controllerGroup: config.controllerGroup });
    return json(h, { result: true, data: idSummary(toLegacyFlashIdInfo(result, id, lang, config.extraUrls)) });
  }

  return json(h, { result: false, message: "Not found" });
}
...
export function createFdServer(options: FdServerOptions = {}): FdServerApp {
  ...
  server.route({
    method: "OPTIONS",
    path: "/{p*}",
    handler: (_request: Request, h: ResponseToolkit) => json(h, undefined, 204)
  });

  server.route({
    method: "GET",
    path: "/{p*}",
    handler: (request: Request, h: ResponseToolkit) => handleLegacyRequest(engine, config, request, h)
  });
